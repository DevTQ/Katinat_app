...
export default function OrderPaidListPage() {
    ...
    try {
      ...
      while (hasMorePages) {
        ...
        if (res?.orders && res.orders.length > 0) {
          const paidOrders = res.orders.filter(order => order.status === 'PAID');
          if (paidOrders.length > 0) {
            allPaidOrders = [...allPaidOrders, ...paidOrders];
          }
      ...
      const totalPaidPages = Math.ceil(filteredOrders.length / limit);

      setOrders(paginatedOrders);
      setTotalPages(totalPaidPages);
    } catch (error) {
  ...
  const columns = [
    ...
    },
    {
      title: 'Trạng thái',
      key: 'status',
      render: () => <Tag color="processing">Đã thanh toán</Tag>
    },
    {
      title: 'Thao tác',
      key: 'action',
  ...
  return (
    <div style={{ padding: '24px' }}>
      <Title level={2}>Đơn hàng đã thanh toán</Title>
